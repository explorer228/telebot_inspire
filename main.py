...
def hello_keyboard(message):
    ...
    user_markup.row('text', 'somethin new', 'voice', 'location')
...
def handle_start(message):
    ...
    answer = 'so, what is next?'
    bot.send_message(message.from_user.id, answer, reply_markup=remove_markup)
...
def handle_text(message):
    if message.text == 'photo':
        ...
        bot.send_location(message.from_user.id, 50.920422660760394, -0.7642660086146799)
        ...
        answer = 'i dont know how to play this game :('
        log(message, answer)
        bot.send_message(message.from_user.id, answer)
